// ==================== REFACTORED InvoiceEditPage.jsx ====================
...
const InvoiceEditPage = ({ sidebarVisible = false }) => {
    const { id } = useParams();
    const currentInvoiceId = id;
    ...
    useEffect(() => {
        if (clients.length > 0 && banks.length > 0 && mfsAccounts.length > 0 && currentInvoiceId) {
            fetchInvoiceData();
        }
    }, [clients, banks, mfsAccounts, currentInvoiceId]);
    ...
    const addPaymentMethod = () => {
        ...
        if (currentPaymentMethod.payment_method_type_id == 1) {
            if (!currentPaymentMethod.bank_id) {
                showAlert("Please select a bank", "warning");
                return;
            }
        }
        if (currentPaymentMethod.payment_method_type_id == 2) {
    ...
    const handleClientSelect = (selectedOption) => {
        if (selectedOption) {
            const selectedClient = clients.find((c) => c.client_id === selectedOption.value);
            if (selectedClient) {
                setFormData(prev => ({
                    ...prev,
    ...
    const handleInputChange = (field, value) => {
        ...
        if (field === 'vat' && (value === '' || parseFloat(value) <= 0) && formData.mushak_file_path) {
    ...
    // UPDATED: Fetch invoice data and preload
    const fetchInvoiceData = async () => {
        const token = localStorage.getItem("authToken");
        setFetchingInvoice(true);

        try {
            const response = await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/wintext-invoice/single-data/${currentInvoiceId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json"
                },
            ...
            const invoiceData = Array.isArray(result) ? result[0] : result.data || result;

            setFormData({
                invoice_number: invoiceData.invoice_number || "",
                billing_date: invoiceData.billing_date || new Date().toISOString().split('T')[0],
                ...
                billing_attention: invoiceData.billing_attention || "",
                billing_attention_phone: invoiceData.billing_attention_phone || "",
                amount_in_words: invoiceData.amount_in_words || "",
                payment_instructions: invoiceData.payment_instructions || "",
                billing_start_date: invoiceData.billing_start_date || new Date().toISOString().split('T')[0],
                billing_end_date: invoiceData.billing_end_date || new Date().toISOString().split('T')[0],
                contract_no: invoiceData.contract_no || "",
                vat: 0,
                mushak_file_path: invoiceData.mushak_file_path || ""
            });

            const items = (invoiceData.wintext_invoice_dtl || []).map(item => ({
                id: item.id,
                description: item.description,
                sms_qty: parseFloat(item.sms_qty),
            ...
            const vatPercentage = subtotal > 0 ? (parseFloat(invoiceData.vat || 0) / subtotal * 100).toFixed(2) : 0;
            setFormData(prev => ({ ...prev, vat: parseFloat(vatPercentage) }));

            const paymentMethods = (invoiceData.wintext_inv_pmnt_instr_dtl || []).map(pm => {
                if (pm.payment_method_type_id == 1) {
                    const bankAccount = banks.find(b => b.id == pm.payment_account_id);
                    return {
                        payment_method_type_id: pm.payment_method_type_id,
                        bank_id: pm.payment_account_id,
                        receiver_name: bankAccount ? bankAccount.receiver_name : pm.receiver_name || "",
                ...
                } else if (pm.payment_method_type_id == 2) {
                    const mfsAccount = mfsAccounts.find(m => m.id == pm.payment_account_id);
                    return {
                        payment_method_type_id: pm.payment_method_type_id,
                        mfs_account_id: pm.payment_account_id,
                        mfs_type_id: mfsAccount ? mfsAccount.mfs_type_id : pm.mfs_type_id || '',
                        receiver_name_mfs: mfsAccount ? mfsAccount.receiver_name : pm.receiver_name || "",
                        receiver_account_name: pm.pmnt_receive_acc || "",
                        bank_id: '',
                        receiver_name: '',
                        pmnt_receive_acc: '',
                        pmnt_rcv_branch: '',
            ...
            showAlert("Failed to fetch invoice data", "danger");
        } finally {
    ...
    const handleUpdateInvoice = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/wintext-invoice/update`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
            ...
            if (Array.isArray(data) && data[2] === "success") {
                showSweetAlert("Invoice updated successfully!", "success");
                setTimeout(() => {
                ...
                const errorMessage = data.message || data[3]?.[0] || "Failed to update invoice";
            ...
            showSweetAlert("Failed to update invoice. Please try again.", "error");
        } finally {
            setLoading(false);
    ...
    return (
        <div style={containerStyle}>
            ...
            <div className='mt-5' style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '7px' }}>
                <h1 style={{ margin: 0, fontSize: '20px', fontWeight: '600' }}>Edit Invoice</h1>
