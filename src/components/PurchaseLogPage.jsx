...
const PurchaseLogPage = ({ sidebarVisible = false }) => {
    ...
    const handleSendSms = async (report) => {
        ...
        try {
            ...
            if (result.status === 'success') {
                showAlert(`SMS sent for ID ${report.id} successfully`, 'success');
            } else {
                        ...
                        {loading ? (
                            ...
                            reports.map((report, i) => (
                                <tr key={report.id} className="align-middle">
                                    ...
                                    <td className="py-1 px-3">
                                        <div style={{ lineHeight: '1.3' }}>
                                            <div style={{ fontSize: '11px' }}>
                                                Merchant: {report.merchant_transaction_id || 'N/A'}
                                            </div>
                                            <div style={{ fontSize: '11px' }}>
                                                EPS: {report.eps_transaction_id || 'N/A'}
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="py-1 px-3">
                                        <div style={{ lineHeight: '1.3' }}>
                                            <div style={{ fontSize: '11px' }}>Request: {formatDisplayDateTime(report.request_time) || 'N/A'}</div>
                                            <div style={{ fontSize: '11px' }}>Transaction: {formatDisplayDateTime(report.transaction_time) || 'N/A'}</div>
                                            <div style={{ fontSize: '11px' }}>Verification: {formatDisplayDateTime(report.verification_time) || 'N/A'}</div>
                                        </div>
                                    </td>
                                    {/* Actions column */}
